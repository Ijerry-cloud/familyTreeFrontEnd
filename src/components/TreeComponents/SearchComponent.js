...
export default function SearchComponent() {
    ...
    const handleChange = (e, value) => {
        let payload_data = {'name': nodes?.results.search_name, 'page': value};
        mutation.mutate({
            url: SEARCH_FAMILY_TREE,
            authenticate: true,
            token,
        ...
        <Container>
            ...
            {mutation?.isSuccess && (
                <Grid>
                    ...
                    <Grid>
                        <TableContainer component={Paper}>
                            <Table sx={{ minWidth: 650 }} aria-label="simple table">
                                ...
                                <TableBody>
                                    {nodes?.results?.tree_list.map((node) => (
                                        <TableRow
                                            key={node.name}
                    ...
                    <Grid>
                        <Stack spacing={2} >
                            <Pagination sx={{ margin: "auto" }} count={nodes?.count} color="primary" onChange={handleChange} page={nodes?.number} />
                        </Stack>
                    </Grid>
